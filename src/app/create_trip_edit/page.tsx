...
export default function ExamplePage() {
  ...
  useEffect(() => {
    const fetchTrip = async () => {
      ...
      try {
        ...
        setSelectedOption(data.tipo === "privada" ? "privada" : "publica");
        setCount(data.quantidadeParticipante || 0);
        setCidadeOrigem(data.cidadeOrigem || "");
        setCidadeDestino(data.cidadeDestino || "");
  ...
  const handleEditTrip = async () => {
    ...
    // Verificar se há erros de validação de data
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const startDateObj = new Date(startDate);
    const endDateObj = new Date(endDate);

    const hasDateErrors = startDateObj < today || startDateObj >= endDateObj;

    if (hasDateErrors) {
      return; // Não prosseguir se houver erros de data
    }
    ...
    const payload = {
      ...
      dataFim: endDate,
      tipo: selectedOption === "publica" ? "publica" : "privada",
  ...
  const handleCloseModal = () => {
    ...
    // Garantir que a viagem atualizada está no localStorage
    const selectedTripStr = localStorage.getItem("selectedTrip");
    if (selectedTripStr) {
      const selectedTrip = JSON.parse(selectedTripStr);
    ...
    <RequireAuth>
      <div className="flex min-h-screen bg-gradient-to-b from-[#1C4CDC] to-[#0F2976]">
        ...
        <div className="flex flex-col items-center w-full bg-gradient-to-b from-[#1C4CDC] to-[#0F2976]">
          <HeaderPages />
          <WhiteBackground titulo="Editar Viagem">
            <div className="max-w-5xl mx-auto px-6 py-8">
              ...
              <div className="text-center">
                ...
                <SuccessModal
                  isOpen={showSuccess}
                  message="Detalhes da Viagem alterado com sucesso!"
