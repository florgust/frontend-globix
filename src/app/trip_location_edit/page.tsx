...
export default function TravelLocation() {
    ...
    useEffect(() => {
        const viagem = localStorage.getItem("viagemEmCriacao");
        if (!viagem) return;
        const { id } = JSON.parse(viagem);

        api.get(`/localizacao/viagem/${id}`)
            .then((res) => {
                if (res.data && res.data.length > 0) {
                    ...
                    setIda({
                        ...
                        dataPartida: loc.idaDataPartida ? loc.idaDataPartida.substring(0, 16) : "",
                        dataChegada: loc.idaDataChegada ? loc.idaDataChegada.substring(0, 16) : "",
                    });
                    setVolta({
                        enderecoPartida: loc.voltaEnderecoPartida,
                        enderecoChegada: loc.voltaEnderecoChegada,
                        dataPartida: loc.voltaDataPartida ? loc.voltaDataPartida.substring(0, 16) : "",
                        dataChegada: loc.voltaDataChegada ? loc.voltaDataChegada.substring(0, 16) : "",
                    });
                }
            })
            .catch((err) => {
    ...
    const handleSave = async () => {
        if (!localizacaoId) {
            alert("Localização não encontrada para atualizar.");
            return;
        }
