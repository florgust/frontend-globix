...
export default function TripBudgetEdit() {
  ...
  // Carregar dados existentes da viagem
  useEffect(() => {
    ...
    api
      .get(`/orcamentos/viagem/${selectedTrip.id}`)
      .then(({ data }) => {
        if (Array.isArray(data) && data.length > 0) {
          // Mapear orçamentos existentes para as categorias
          const updatedCategories = [...initialCategories];

          (data as Orcamento[]).forEach((orc) => {
            const categoryIndex = updatedCategories.findIndex(
              (cat) => cat.name.toLowerCase() === orc.categoria.toLowerCase()
            );

            if (categoryIndex !== -1) {
              updatedCategories[categoryIndex] = {
                ...updatedCategories[categoryIndex],
                id: orc.id ?? updatedCategories[categoryIndex].id,
      ...
      .catch((error) => {
        ...
        setAlertMessage("Erro ao carregar orçamentos existentes.");
        setCategories(initialCategories);
      });
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setAlertMessage("Erro ao salvar orçamentos. Tente novamente.");
    }
  };
    ...
    <RequireAuth>
      <div className="flex min-h-screen bg-gradient-to-b from-[#1C4CDC] to-[#0F2976]">
        ...
        <div className="flex flex-col items-center w-full bg-gradient-to-b from-[#1C4CDC] to-[#0F2976]">
          <HeaderPages />
          <WhiteBackground titulo="Atualizar Orçamento">
