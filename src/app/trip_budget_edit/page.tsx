...
export default function TripBudgetEdit() {
  const [categories, setCategories] =
    useState<{ id?: number; name: string; value: string }[]>(initialCategories);
  const [notes, setNotes] = useState("");
  const [participants, setParticipants] = useState(1);
  const [showSuccess, setShowSuccess] = useState(false);
  const router = useRouter();

  // Dados da viagem para exibir no topo
  let tripData = {
    name: "",
    startDate: "",
    endDate: "",
  ...
  useEffect(() => {
    ...
    tripData = {
      name: selectedTrip.nome ?? "",
      startDate: selectedTrip.dataInicio ?? "",
      endDate: selectedTrip.dataFim ?? "",
      location: selectedTrip.nome?.split(" ")[2] ?? "",
      participants: selectedTrip.quantidadeParticipante ?? 1,
    };
    setParticipants(selectedTrip.quantidadeParticipante ?? 1);

    // Buscar orçamentos existentes
    api.get(`/orcamentos/viagem/${selectedTrip.id}`).then(({ data }) => {
      if (Array.isArray(data) && data.length > 0) {
        setCategories(
          (data as Orcamento[]).map((orc, idx) => ({
            id: orc.id ?? Date.now() + idx,
            name: orc.categoria,
            value: orc.custo.toString(),
          }))
        );
        setNotes((data as Orcamento[])[0]?.observacao || "");
      } else {
  ...
  const handleCategoryNameChange = (id: number | undefined, name: string) => {
    setCategories(
      categories.map((cat) => (cat.id === id ? { ...cat, name } : cat))
    );
  };

  const handleRemoveCategory = async (id: number | undefined) => {
    // Se não tem id, só remove do estado local
    const catToRemove = categories.find((cat) => cat.id === id);
    if (catToRemove?.id && typeof catToRemove.id === "number") {
      try {
        await api.delete(`/orcamento/${catToRemove.id}`);
      } catch {
  ...
  const handleAddCategory = () => {
    setCategories([...categories, { id: Date.now(), name: "", value: "" }]);
  };
  ...
  // Preencher os dados da viagem para o topo
  const selectedTripStr =
    typeof window !== "undefined" ? localStorage.getItem("selectedTrip") : null;
  if (selectedTripStr) {
    const selectedTrip = JSON.parse(selectedTripStr);
    tripData = {
      name: selectedTrip.nome ?? "",
      ...
      endDate: selectedTrip.dataFim ?? "",
      location: selectedTrip.nome?.split(" ")[2] ?? "",
      participants: selectedTrip.quantidadeParticipante ?? participants,
    };
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      for (const cat of categories) {
        ...
        // Considere como novo se id não existe ou é um id local (ex: Date.now() > 1_000_000_000_000)
        if (cat.id && cat.id < 1_000_000_000_000) {
          // PUT: orçamento já existe no banco
          await api.put(`/orcamento/${cat.id}`, {
            categoria: cat.name,
          ...
          const response = await api.post("/orcamento", {
            viagemId: selectedTrip.id,
            categoria: cat.name,
            custo: parseFloat(cat.value),
            observacao: notes || "Orçamento destinado à viagem",
          });
          // Atualize o id local para o id real retornado pelo backend
    ...
    <RequireAuth>
      <div className="flex min-h-screen bg-gradient-to-b from-[#1C4CDC] to-[#0F2976]">
        ...
        <div className="flex flex-col items-center w-full bg-gradient-to-b from-[#1C4CDC] to-[#0F2976]">
          <HeaderPages />
          <h1 className="font-bold text-4xl text-left text-white w-full pl-22 mt-4">Editar Orçamento da Viagem</h1>
